...
class Tranner:
    ...
    def train(self,max_itr=20,max_batch=2000,max_epoch=1,save_path='./test11.pt'):
        ...
        for epoch in range(max_epoch):
            for i, batch in enumerate(data_loader):
                ...
                for itr in range(max_itr):
                    self._train_one_batch(batch['rgb'],batch['direction'],batch['points'])
                print('This is',itr+1,'/',max_itr,' iteration,',i+1,'/',max_batch,'batch',epoch+1,'/',max_epoch,'epoch')
            
            self._save_ckpt_each_epoch(epoch,max_epoch,save_path)
    ...
    def test_one_picture(self):
        ...
        for i, batch in enumerate(data_loader):
            ...
            # with Timing('plt'):
                cv2.imwrite(file_name, cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR))
                PSNR=self.PSNR(file_name,"./L1_Data_Manager/Replica/office0/results/frame000000.jpg")
                print('PSNR',PSNR)
                break
                # break
        
    def _train_one_batch(self,rgb,direction,points):
    ...
    def _save_ckpt_each_epoch(self,epoch,max_epoch,save_path):
        file_name="this_is_{}of{}epoch.pt".format(epoch+1,max_epoch)
        savehere=os.path.join(save_path,file_name)
        print(savehere)
        torch.save(self.grid_encoding.memory,savehere)
        try:
            torch.save(self.grid_encoding.memory,savehere)
        except:
            print("Warning: This epoch is not saved, please break and check if you are not mean to do that")
        
        
        file_name_last_epoch="this_is_{}of{}epoch.pt".format(epoch,max_epoch)
        savehere_last=os.path.join(save_path,file_name_last_epoch)
        try:
            os.remove(savehere_last)
        except:
            if epoch!=0:
                print("Warning: Last epoch didn't save")
